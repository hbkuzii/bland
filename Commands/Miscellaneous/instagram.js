    ...
    async execute(message, args) {
        try {
            ...
            const embed = new EmbedBuilder({
                ...
                title: `> ${user.display_name !== user.username ? `${user.display_name} (@${user.username})` : `${user.username}`} ${
                    user.statistics.verified ? '☑️' : ''
                } ${user.statistics.private ? ':lock:' : ''}`,
...
async function serverRevision() {
    try {
        const response = await axios.get('https://www.instagram.com/accounts/login/', { responseType: 'text' });
